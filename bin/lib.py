...
def load_confs(confs_path=os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'))+'/confs/config.yaml'):
    # TODO Docstring
    global CONFS
...
def archive_dataset_schemas(step_name, local_dict, global_dict):
    ...
    # Reference variables
    #data_schema_dir: ../data/schema --> extract.csv and transform.csv
        #extract.csv contains variable : file_path,extension, text
        #transform.csv=extract.csv + candidate_name, email, phone, experience ..., open-source
    ...
    data_sets = filter(lambda kv: type(kv[1]) == pd.DataFrame, env_variables.items())
...
def birthdate_match(string_to_search, term):
    """
    A utility function which return the first match to the `regex_pattern` in the `string_to_search`
    :param string_to_search: A string which may or may not contain the term.
    :type string_to_search: str
    ...
    try:
        regular_expression = re.compile(term, re.IGNORECASE)
        date = re.findall(regular_expression, string_to_search)
        not_date=[]
        for k in date:
